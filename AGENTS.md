...
- **Web presentation.** `apps/web` uses Tailwind with Yard's semantic design tokens, project-owned shadcn/ui components backed by Base UI when needed, Phosphor Icons, and Motion for React. Use shared motion tokens and reduced-motion behavior; use CSS for simple transitions. Do not add a second UI kit or competing icon/animation library.
